...
import { Listbox, ListboxButton, ListboxOption, ListboxOptions } from '@headlessui/react';
        ...
        const options = [
            { id: 'agentStatus', name: 'Agent Individual Status' },
            { id: 'cdrReport', name: 'Segments Reports' },
            { id: 'interactionReport', name: 'Interactions Reports' },
        ...
        export const WAITING_COLUMN_MAP: { [key: string]: string } = {
            'queueCDR': 'startTime',
            'queueCDRLegs': 'waitTime',
            'sipLatency': 'endTime', // Según tu ejemplo original
            'sipLatencyTotals': 'startTime',
            'queueADR': 'startTime',
            'userGroups': 'null', // O el campo que use para un reporte basado en el tiempo
        ...
        export const QUERY_TEMPLATES = {
            ...
            cdrReport: (server: string, database: string, table: string, timeColumn: string, waitColumn: string, startDate: string, endDate: string, limit: number, offset:number) => ({
                ...
                payload: {
                    ...
                    q: `
                        SELECT
                        queueCallManagerID, ${timeColumn}, endTime, agentName, ${waitColumn}, agentTalkTime, agentHoldTime, wrapup, segmentNumber, queueName, username, transferToData, commType
            ...
            interactionReport: (server: string, database: string, table: string, timeColumn: string, waitColumn: string, startDate: string, endDate: string, limit: number, offset:number) => ({
                endpoint: "V2/query/", // Sigue usando el endpoint genérico para consultas SQL
                payload: {
                    method: "query", // Asegúrate de que el método sea 'query' para las consultas SQL
                    q: `
                        SELECT 
                        agentName, queueCallManagerID, ${timeColumn}, endTime, queueName, hangupCause, agentName, cidNumber, cidName, inboundName, queueSkills, queueCallManagerID, inboundNumber, holdTime, talkTime, queueCallback, commType, SEC_TO_TIME(talkTime+holdTime+wrapUpTime) AS "AHT", COUNT(queueCallManagerID) AS "Total Calls", wrapUpTime 
                        FROM ${server}.${database}.${table}
                        WHERE endTime BETWEEN '${startDate}' AND '${endDate}'
                        LIMIT 20000
                    `,
...
const DOWNLOAD_BATCH_SIZE = 10000; 
    ...
    useEffect(() => {
        ...
        if (selectedQueryTemplate === 'liveStatus') { // Creamos una nueva plantilla para la consulta directa
            ...
            const waitColumn = WAITING_COLUMN_MAP[table];

            const { endpoint, payload } = QUERY_TEMPLATES.cdrReport(server, database, table, timeColumn, waitColumn, startDate, endDate, itemsPerPage, currentOffset);
                apiEndpoint = endpoint;
                apiPayload = payload;
        }

        else if (selectedQueryTemplate === 'interactionReport') {
            if (!isValidRange()) {
                setLoading(false);
                return;
            }
            ...
            const { endpoint, payload } = QUERY_TEMPLATES.interactionReport(
                server,
                database,
                table,
                timeColumn,
    ...
    try {
        ...
        else if (selectedQueryTemplate === "liveStatus" || selectedQueryTemplate === "cdrReport" || selectedQueryTemplate === "interactionReport") {
        ...
        try {
            ...
            for (let page = 0; page < totalPagesToFetch; page++) {
             ...
             const timeColumn = TIME_COLUMN_MAP[table];
                ...
                const waitColumn = WAITING_COLUMN_MAP[table];
                if (!waitColumn) {
                    setError(`No se encontró una columna de espera para la tabla: ${table}`);
                    setLoading(false);
                    return;
                }
                const { endpoint, payload } = QUERY_TEMPLATES.cdrReport(
                    server, database, table, timeColumn, waitColumn, startDate, endDate, DOWNLOAD_BATCH_SIZE, offset
                );
    ...
    useEffect(() => {
        ...
        <div className='p-6 max-w-screen-xl mx-auto mt-[100px]'>
            ...
            {/* --- SECCIÓN DE SELECCIÓN DE DATOS (CONDICIONAL SEGÚN PLANTILLA/CUSTOM QUERY) --- */}
                    ...
                    {(selectedQueryTemplate === 'cdrReport' || selectedQueryTemplate === 'interactionReport') && (
